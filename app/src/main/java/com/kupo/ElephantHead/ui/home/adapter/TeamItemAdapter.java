 ...
 * 作者：
 */
public class TeamItemAdapter extends BaseMultiItemQuickAdapter<TeamListModel.DataBean.RecordsBean, BaseViewHolder> {

    private Activity mActivity;

    public TeamItemAdapter(List<TeamListModel.DataBean.RecordsBean> data, Activity mActivity) {
        super(data);
    ...
    protected void convert(BaseViewHolder baseViewHolder, TeamListModel.DataBean.RecordsBean recordsBean) {
        switch (baseViewHolder.getItemViewType()) {
            case 0:
                baseViewHolder.setText(R.id.team_item_userName, recordsBean.getName())
                        .setText(R.id.team_item_time, recordsBean.getTime())
                        .setText(R.id.team_item_num, recordsBean.getDirectCount() + "/" + recordsBean.getTotalCount());
                break;
        }
    }
}
